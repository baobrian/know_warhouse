...
def cohort_register(pk,df):
    ...
    data=df[pk]
    data['draw_value']=1
    data['re_appl_value']=data['first_tx_distr_dt'].map(lambda x: 1 if not pd.isnull(x) else 0)
    total_reg=data.groupby(['first_tx_distr_dt']).size()
    total_reg.name='reg_total'
    regist['total']=total_reg
    reg_temp=pd.pivot_table(data=data,values='intrv',\
               index='first_tx_distr_dt',columns='int_intval',\
               aggfunc=np.sum)
...
if __name__ == '__main__':
    path='C:\Users\Administrator\Desktop\liushi\data20190625\data\\'
    ...
    os.chdir('C:\Users\Administrator\Desktop\liushi\data20190625\\')
    result_cohort.to_excel('re_appl.xlsx',header=True)
